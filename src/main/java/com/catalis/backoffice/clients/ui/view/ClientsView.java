...
import com.catalis.backoffice.api.service.customer.CustomerService;
...
public class ClientsView extends VerticalLayout implements HasUrlParameter<String> {
    ...
    public ClientsView(CustomerService customerService) {
        ...
        this.tabs = new Tabs(clientsTab, documentsTab);
    ...
    private void loadMockClientData() {
        try {
            ...
            // Convert CustomerDTOs to Clients
            List<Client> clients = new ArrayList<>();
            for (CustomerDTO customer : customers) {
                clients.add(convertToClient(customer));
            }

            // If no customers were found, generate fallback mock data
            if (clients.isEmpty()) {
                clients = generateFallbackMockData();
            }

            // Set the items in the grid
            clientGrid.setItems(clients);
        } catch (Exception e) {
            // If there's an error, generate fallback mock data
            List<Client> fallbackClients = generateFallbackMockData();
            clientGrid.setItems(fallbackClients);
    ...
    /**
     * Generates fallback mock data when the service is not available.
     * 
     * @return a list of mock clients
     */
    private List<Client> generateFallbackMockData() {
        List<Client> clients = new ArrayList<>();
    ...
    /**
     * Converts a CustomerDTO to a Client.
     * 
     * @param customerDTO the CustomerDTO to convert
     * @return the converted Client
     */
    private Client convertToClient(CustomerDTO customerDTO) {
        ...
        // These fields don't exist in CustomerDTO, so we'll use default values
        String segment = "Retail";
        String status = "Active";

        return new Client(id, name, email, phone, address, segment, status);
    }
