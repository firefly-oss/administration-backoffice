...
@Route(value = "risk-model", layout = MainLayout.class)
@PageTitle("Risk Model")
public class RiskModel extends SplitViewFrame {
    ...
    public RiskModel(RiskAndComplianceService riskAndComplianceService) {
    ...
    private Grid createGrid() {
        ...
        dataProvider = DataProvider.ofCollection(riskAndComplianceService.getRiskModels());
        ...
        grid.addColumn(new ComponentRenderer<>(this::createRiskScore))
                .setAutoWidth(true)
                .setFlexGrow(0)
    ...
    private Component createUserInfo(Person person) {
        ListItem item = new ListItem(
                ...
                "Risk model for " + person.getEmail());
    ...
    private Component createRiskScore(Person person) {
        int score = person.getRandomInteger() % 100;
        Span span = new Span(Integer.toString(score));

        // Apply color based on risk score
        if (score < 30) {
            span.getElement().getStyle().set("color", "var(--lumo-success-color)");
        } else if (score < 70) {
            span.getElement().getStyle().set("color", "var(--lumo-primary-color)");
        } else {
            span.getElement().getStyle().set("color", "var(--lumo-error-color)");
        }
    ...
    private DetailsDrawer createDetailsDrawer() {
        ...
        footer.addSaveListener(e -> {
            detailsDrawer.hide();
            UIUtils.showNotification("Risk model updated.");
        });
    ...
    private void showDetails(Person person) {
        detailsDrawerHeader.setTitle("Risk Model: " + person.getName());
    ...
    private FormLayout createDetails(Person person) {
        ...
        modelName.setValue(person.getName());
        modelName.setWidthFull();

        TextField description = new TextField();
        description.setValue("Risk model for " + person.getEmail());
        ...
        riskType.setItems("Credit Risk", "Market Risk", "Operational Risk", "Liquidity Risk", "Compliance Risk");
        riskType.setValue("Credit Risk");
        riskType.setWidthFull();

        TextField threshold = new TextField();
        threshold.setValue(Integer.toString(person.getRandomInteger() % 100));
        ...
        form.setResponsiveSteps(
                ...
                new FormLayout.ResponsiveStep("1024px", 3,
                        FormLayout.ResponsiveStep.LabelsPosition.TOP));
        form.addFormItem(modelName, "Model Name");
        form.addFormItem(description, "Description");
        form.addFormItem(status, "Status");
        form.addFormItem(riskType, "Risk Type");
        form.addFormItem(threshold, "Risk Threshold");
        return form;
    }
